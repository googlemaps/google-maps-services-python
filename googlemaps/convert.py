...
def markers(arg):
    """Converts a dict of marker descriptors to the format expected by
    ...
    :param arg: The marker descriptors.
    :type arg: dict
    ...
    if isinstance(arg, dict):
        ...
        for descriptor in ['size', 'color', 'label']:
            if descriptor in arg:
                params.append("%s:%s" % (descriptor, arg[descriptor]))

        if 'locations' in arg:
            params.append(location_list(arg['locations']))

        return join_list('|', params)

    raise TypeError(
        "Expected a markers dict, "
        "but got %s" % type(arg).__name__)


def paths(arg):
    """Converts a dict of path parameters to the format expected by
    the Google Maps server.
    ...
    if isinstance(arg, dict):
        ...
        for parameter in ['weight', 'color', 'fillcolor', 'geodesic']:
            if parameter in arg:
                params.append("%s:%s" % (parameter, arg[parameter]))

        if 'points' in arg:
            params.append(location_list(arg['points']))

        return join_list('|', params)

    raise TypeError(
        "Expected a path dict, "
        "but got %s" % type(arg).__name__)
